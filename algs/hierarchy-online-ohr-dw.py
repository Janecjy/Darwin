...
class OnlineHierarchy:
    ...
    def calculateAvgSize(self):
        for i in range(len(self.bucket_count)):
            if i == 0:
                self.round_avg_size += BUCKET_LIST[i]/2*self.bucket_count[i]
            else:
                if i == len(self.bucket_count)-1:
    ...
    def feedRequest(self, t, id, size):
        ...
        if self.current_stage == OnlineStage.EXPERT_BANDIT:
            ...
            if self.round_request_num == BANDIT_ROUND_LENGTH:
                ...
                self.updateReward()
                self.round += 1
                self.round_request_num = self.round_hoc_hit_num = self.round_hoc_hit_num_all = self.round_dw = 0
                self.selectArm()
                # t2 = datetime.datetime.now()
...
def main():
    ...
    print('hoc hit: {:.4f}%, hr: {:.4f}%, bmr: {:.4f}%, disk read: {:f}, disk write: {:f}, ohr dw comb: {:f}'.format(cache.tot_hoc_hit/cache.tot_req_num*100, cache.tot_obj_hit/cache.tot_req_num*100, cache.tot_byte_miss/cache.tot_req_bytes*100, cache.tot_disk_read, cache.tot_disk_write, cache.tot_hoc_hit/cache.tot_req_num*100 - cache.tot_disk_write*1e-7))
    return 0
