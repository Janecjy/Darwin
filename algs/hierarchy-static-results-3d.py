...
trace_path = output_dir = freq_thres = size_thres = rec_thres = hoc_s = dc_s = None
...
def parseInput():
    ...
    global trace_path, output_dir, freq_thres, size_thres, rec_thres, hoc_s, dc_s
    ...
    try:
        ...
        opts, args = getopt.getopt(argv, 't:o:f:s:r:h:d:', ['trace_path', 'output_dir', 'freq_thres', 'size_thres', 'rec_thres', 'hoc_size', 'dc_size'])
        # Check if the options' length is 3
        if len(opts) != 7:
            print('usage: hierarchy.py -t <trace_path> -o <output_dir> -f <frequency_threshold> -s <size_threshold> -f <recency_threshold> -h <HOC_size> -d <DC_size>')
        else:
...
def request(t, id, size):
    ...
    if id in hoc:
        ...
        if size < size_thres:
            ...
            if countFreq(id) == freq_thres and tot_num - rec_table[id] < rec_thres:
                promote(id, size)
...
def run():
    ...
    firstWarmup = True
    size_table = {} # id: size
    global rec_table # id: last_access request count
    # reqs = []
    # freqs = []
    # hits = [] # [(size, hit)]
...
def main():
    parseInput()
    if None not in (trace_path, freq_thres, size_thres, rec_thres, hoc_s, dc_s):
        print('trace: {}, freq: {}, size: {}, rec: {}, HOC size: {}, DC size: {}'.format(trace_path, freq_thres, size_thres, rec_thres, hoc_s, dc_s))
    else:
        sys.exit(2)
