...
input_dir = "/mydata/traces-production/"
...
for trace in os.listdir(input_dir):
    ...
    thresholds = [10, 20, 50, 5000, 10000]
    counts = [0, 0, 0, 0, 0]
    sorted_sizes = np.sort(sizes)
    tot_req = len(sorted_sizes)
    for s in sorted_sizes:
        for i, t in enumerate(thresholds):
            if s < t:
                counts[i] += 1
    counts = [x/tot_req*100 for x in counts]
    print(counts)
