...
def main():
    ...
    last_real_time = round(time.time()/1000000000)
    last_trace_time = 0
    ...
    for line in open(trace_path):
        ...
        id = int(line[1])
        size = int(line[2])
        current_real_time = round(time.time()/1000000000)
        if (t - last_trace_time > current_real_time - last_real_time):
            # print("sleep: ", t - last_trace_time- (current_real_time - last_real_time))
            # sys.stdout.flush()
            time.sleep(0.001 *(t - last_trace_time- (current_real_time - last_real_time)))
