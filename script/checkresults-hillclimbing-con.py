...
def countStat(dirPath):
    ...
    for root, dirs, files in os.walk(dirPath):
        for file in files:
            ...
            c = int(file.split('.')[0].split('-')[1].replace("c", ""))
            trace = root.split('/')[-1]
            ...
            if file_res:
                ...
                if file.endswith(".txt"):
                    trace_stats[trace]['c'+str(c)] = [x[0] for x in file_res][-1]
    print(trace_stats)  
    pickle.dump(trace_stats, open("/mydata/output-hillclimbing-con/results.pkl", "wb"))
    
if __name__ == "__main__":
    countStat("/mydata/output-hillclimbing-con/")