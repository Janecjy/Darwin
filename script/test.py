...
import pickle
cluster_model = pickle.load(open("/mydata/results/kmeans_2.pkl", "rb"))
cluster_result = pickle.load(open("/mydata/results/cluster_experts_2.pkl", "rb"))
expert_list = []
...
import numpy as np
def getPotentialExperts(features):

    X_dist = cluster_model.transform([features])[0]
...
# check if the real best experts are in the cluster
from collections import defaultdict
import pickle
feature_set = ['iat_avg', 'sd_avg', 'size_avg']
name_list = []
feature_list = []
feature_files = [path for path in os.listdir("/mydata/features/test-set-real")]
best_results = pickle.load(open("/mydata/results/best_result.pkl", "rb"))
count_list = []
for file in feature_files:
    feature = []
    name = file.split('.')[0]
    features = pickle.load(open("/mydata/features/test-set-real/"+file, "rb"))
    # print(features.keys())
    for k in feature_set:
        ...
        if type(v) is dict or type(v) is defaultdict:
            ...
            feature.append(v)
    if file.startswith("tc-0-1-5:260"):
        print(feature)
    p = getPotentialExperts(feature)
    r = best_results[name]
...
# check the cluster expert reduction

import matplotlib.pyplot as plt
import numpy as np

# sort the data:
# print(len(wrong_data[0]))
p = 1. * np.arange(len(count_list)) / (len(count_list) - 1)
fig = plt.figure()
plt.plot(sorted(count_list), p)