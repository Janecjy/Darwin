...
do
    ...
    if [[ $NAME =~ $regex ]]
    then
        echo "python3 ./algs/hierarchy-online.py -t "$TRACE" -m "$BASE_DIR"tragen-models-"$r"x -h "$((100000 * r))" -d "$((10000000 * r))" > "$OUTPUT_DIR$NAME".txt" >> "./script/tacc_scripts/genOnline/genOnline"$r"x_job"
    fi
done
